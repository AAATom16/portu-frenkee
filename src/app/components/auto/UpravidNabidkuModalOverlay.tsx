...
// ─── Coverage card (Povinné ručení sub-section) ───────────────────────────────
function CoverageCard({ level, description, range, selected }: { level: string; description: string; range: string; selected?: boolean }) {
  const bars = [
    { height: 28, filled: true },
    { height: 40, filled: level === "Střední" || level === "Vysoké" },
    { height: 56, filled: level === "Vysoké" },
  ];
  return (
    <div className={`flex-1 min-w-0 relative rounded-[8px] py-[10px] px-[8px] flex flex-col items-center gap-[8px]`}>
      {/* bars */}
      <div className="flex gap-[3px] items-end h-[60px]">
        {bars.map((bar, i) => (
          <div key={i} className="w-[24px] flex items-end" style={{ height: "60px" }}>
            {bar.filled ? (
              <div className="bg-[#00A878] rounded-[4px] w-full" style={{ height: `${bar.height}px` }} />
            ) : (
              <div className="relative rounded-[4px] w-full" style={{ height: `${bar.height}px` }}>
                <div aria-hidden="true" className="absolute border-[3px] border-[#00A878] border-solid inset-0 pointer-events-none rounded-[4px]" />
              </div>
            )}
          </div>
        ))}
      ...
      <p className="font-['Inter',sans-serif] text-[#1A1A1A] text-[14px] text-center leading-[1.4]">{level}</p>
      <p className="font-['Inter',sans-serif] text-[#1A1A1A] text-[12px] text-center leading-[1.4]">{description}</p>
      <p className="font-['Inter',sans-serif] text-[#999999] text-[11px] text-center leading-none">{range}</p>
      <div aria-hidden="true" className={`absolute border border-solid inset-0 pointer-events-none rounded-[8px] ${selected ? "border-[#64748b]" : "border-[#e2e9f0]"}`} />
    </div>
  );
}

// ─── Spoluúčast row ───────────────────────────────────────────────────────────
function SpoluucastRow() {
  return (
    <div className="content-stretch flex items-center justify-between relative shrink-0 w-full">
      <p className="font-['Inter',sans-serif] text-[#1A1A1A] text-[18px] leading-[1.6]" style={{ fontWeight: 700 }}>Spoluúčast</p>
      <div className="content-stretch flex gap-[8px] items-center shrink-0 w-[260px]">
        <div className="bg-white h-[40px] relative rounded-[8px] shrink-0">
          <div className="content-stretch flex gap-[8px] h-full items-center overflow-clip p-[4px] relative rounded-[inherit]">
            <div className="bg-[#00A878] content-stretch flex gap-[8px] h-full items-center overflow-clip px-[12px] relative rounded-[6px] shrink-0">
              <p className="font-['Inter',sans-serif] text-[14px] text-white leading-[1.6]">%</p>
            </div>
            <p className="font-['Inter',sans-serif] text-[#94a3b8] text-[14px] leading-[1.6] w-[35px]">Fixní</p>
          </div>
          <div aria-hidden="true" className="absolute border border-[#e2e9f0] border-solid inset-0 pointer-events-none rounded-[8px]" />
        </div>
        <div className="flex-1 min-w-0"><SelectInput value="10 %" /></div>
      </div>
    </div>
  );
}
...
// ─── Hodnota vozu row ─────────────────────────────────────────────────────────
function HodnotaVozuRow() {
  return (
    <div className="content-stretch flex items-center justify-between relative shrink-0 w-full">
      <p className="font-['Inter',sans-serif] text-[#1A1A1A] text-[18px] leading-[1.6]" style={{ fontWeight: 700 }}>Hodnota vozu</p>
      <div className="content-stretch flex gap-[8px] items-center shrink-0 w-[260px]">
        <div className="bg-white flex-[1_0_0] h-[40px] min-h-px min-w-px relative rounded-[8px]">
          <div className="flex flex-row items-center overflow-clip rounded-[inherit] size-full">
            <div className="content-stretch flex gap-[8px] items-center px-[12px] relative size-full">
              <p className="flex-1 min-w-0 font-['Inter',sans-serif] text-[#94a3b8] text-[14px] leading-[1.6]">0</p>
        ...
        <button
          type="button"
          className="h-[40px] px-[12px] rounded-[8px] shrink-0 cursor-pointer hover:opacity-90 transition-opacity flex items-center whitespace-nowrap"
          style={{ backgroundColor: "#00A878" }}
        >
          <span className="font-['Inter',sans-serif] text-[13px] text-white leading-[1.6]">Ocenit vůz</span>
        </button>
      </div>
    </div>
  );
...
function InsuranceTypeRow({
  ...
  return (
    <div
      className="bg-[#f8fafc] relative rounded-[12px] shrink-0 w-full cursor-pointer select-none"
      onClick={onToggle}
    >
      ...
      <div
        ...
        className={`absolute border border-solid inset-0 pointer-events-none rounded-[12px] transition-all ${
          enabled
            ? "border-[#E5E7EB] shadow-[0px_0px_10.3px_0px_rgba(0,168,120,0.12)]"
            : "border-[#e2e9f0]"
        }`}
      />
    </div>
...
export default function UpravidNabidkuModalOverlay({ onClose }: UpravidNabidkuModalOverlayProps) {
  // Toggle states matching the values from the calculation
  const [povinneRuceni, setPovinneRuceni] = useState(true);
  const [havarijni, setHavarijni] = useState(true);
  const [skla, setSkla] = useState(true);
  const [rozsirenaAsistence, setRozsirenaAsistence] = useState(true);
  const [strezZveri, setStrezZveri] = useState(true);
  const [zivelnariziha, setZivelnariziha] = useState(false);
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-[24px] pt-[24px] pb-[16px]">
          ...
          <button
            onClick={onClose}
            className="flex items-center justify-center w-[32px] h-[32px] rounded-full hover:bg-[#f8fafc] transition-colors text-[#94a3b8] hover:text-[#00A878]"
            aria-label="Zavřít"
          >
        ...
        <div className="flex flex-col gap-[20px] p-[24px]">
          ...
          <div className="flex flex-col gap-[8px]">
            ...
            <InsuranceTypeRow
              ...
              enabled={povinneRuceni}
              onToggle={() => setPovinneRuceni((v) => !v)}
            />
            {povinneRuceni && (
              <div className="flex flex-col gap-[10px] pt-[4px] pb-[4px] px-[4px]">
                <p className="font-['Inter',sans-serif] text-[#999999] text-[13px] leading-[1.5]">
                  Vyberte úroveň krytí povinného ručení:
                </p>
                <div className="flex gap-[12px] items-stretch w-full">
                  <CoverageCard level="Nízké" description="Jezdím občas a stačí mi základní krytí." range="(50 mil. – 75 mil.)" />
                  <CoverageCard level="Střední" description="Zlatá střední cesta pro každodenní ježdění." range="(75 mil. – 150 mil.)" selected />
                  <CoverageCard level="Vysoké" description="Chci být opravdu v klidu nebo jezdím hodně." range="(150 mil. – 300 mil.)" />
                </div>
              </div>
            )}
            ...
            <InsuranceTypeRow
              ...
              enabled={havarijni}
              onToggle={() => setHavarijni((v) => !v)}
            />
            {havarijni && (
              <div className="flex flex-col gap-[14px] pt-[4px] pb-[4px] px-[4px]">
                {/* Rozsah */}
                <div className="content-stretch flex items-center justify-between relative shrink-0 w-full">
                  <p className="font-['Inter',sans-serif] text-[#1A1A1A] text-[18px] leading-[1.6]" style={{ fontWeight: 700 }}>Rozsah</p>
                  <div className="w-[260px]"><SelectInput value="All risk" /></div>
                </div>
                <SpoluucastRow />
                <HodnotaVozuRow />
              </div>
          ...
          <div className="flex flex-col gap-[12px]">
            ...
            <AddonRow
              ...
              price="+ 310 Kč"
              enabled={skla}
              onToggle={() => setSkla((v) => !v)}
              bullets={[
                "Limit 15 000 Kč, spoluúčast 10%",
                "Smluvní servis bez spoluúčasti",
              ]}
            />

            {skla && <div className="h-px bg-[#f0edf8] w-full" />}

            <AddonRow
              label="Rozšířená asistence"
              price="+ 310 Kč"
              enabled={rozsirenaAsistence}
              onToggle={() => setRozsirenaAsistence((v) => !v)}
              bullets={["Limit 100 000 Kč"]}
            />

            {rozsirenaAsistence && <div className="h-px bg-[#f0edf8] w-full" />}

            <AddonRow
              label="Střez se zvěří"
              price="+ 732 Kč"
              enabled={strezZveri}
              onToggle={() => setStrezZveri((v) => !v)}
              bullets={[
                "Limit 100 000 Kč",
                "Území Evropy a Turecka",
              ]}
            />

            {strezZveri && <div className="h-px bg-[#f0edf8] w-full" />}

            <AddonRow
              label="Živelná rizika"
              price="+ 420 Kč"
              enabled={zivelnariziha}
              onToggle={() => setZivelnariziha((v) => !v)}
              bullets={[
                "Limit 100 000 Kč, spoluúčast 10%",
                "Území Evropy a Turecka",
              ]}
            />
          </div>
